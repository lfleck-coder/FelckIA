...
function processProdData(txt, fname) {
  ...
  // Merge: mantém histórico acumulado de uploads anteriores.
  // Para cada WO, preserva o maior DPU já registrado (mais conservador para qualidade).
  ...
  try { localStorage.setItem('aq_wo_map', JSON.stringify(WO_MAP)); } catch(e) {}

  PROD = qg09;
  RFT_TARGET = calcAutoTarget();

  var totalWOs = Object.keys(WO_MAP).length;
  document.getElementById('fb-prod').innerHTML =
    '<div class="al al-ok"><strong>' + fname + '</strong> carregado &middot; ' +
    rows.length + ' linhas &middot; QG09: <strong>' + qg09.length + '</strong> novos registros &middot; ' +
    '<strong>' + totalWOs + '</strong> WOs no historico acumulado' +
    '<div style="margin-top:8px">' +
    '<button class="btn btnsm btnd" onclick="limparHistoricoRFT()" style="font-size:10px">Limpar historico RFT</button>' +
    '</div></div>';

  document.getElementById('rft-content').style.display = 'block';
  setTimeout(function() { renderRFT(); }, 50);
}

// ── Limpa o histórico acumulado de WOs ───────────────────────
function limparHistoricoRFT() {
  if (!confirm('Limpar todo o historico RFT acumulado? Esta acao nao pode ser desfeita.')) return;
  WO_MAP = {};
  try { localStorage.removeItem('aq_wo_map'); } catch(e) {}
  document.getElementById('fb-prod').innerHTML =
    '<div class="al al-w">Historico RFT limpo. Faca um novo upload para comecar.</div>';
  document.getElementById('rft-content').style.display = 'none';
}
